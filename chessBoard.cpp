...
int CheckBoard(const CHESS_BOARD *pos) {
	...
	ASSERT(t_minPce[WHITE] == pos->minorPieceNum[WHITE] && t_minPce[BLACK] == pos->minorPieceNum[BLACK]);
	ASSERT(t_majPce[WHITE] == pos->majorPieceNum[WHITE] && t_majPce[BLACK] == pos->majorPieceNum[BLACK]);
	ASSERT(t_bigPce[WHITE] == pos->bigPieceNum[WHITE] && t_bigPce[BLACK] == pos->bigPieceNum[BLACK]);
...
void UpdateListsMaterial(CHESS_BOARD *pos) {
	...
	for (index = 0; index < BOARD_SQUARE_NUM; ++index) {
		...
		if (piece != OFFBOARD && piece != EMPTY) {
			...
			if (PieceBig[piece] == TRUE) pos->bigPieceNum[colour]++;
			if (PieceMin[piece] == TRUE) pos->minorPieceNum[colour]++;
			if (PieceMaj[piece] == TRUE) pos->majorPieceNum[colour]++;
...
int ParseFen(char *fen, CHESS_BOARD *pos) {
...
void ResetBoard(CHESS_BOARD *pos) {
	...
	for (index = 0; index < 2; ++index) {
		pos->bigPieceNum[index] = 0;
		pos->majorPieceNum[index] = 0;
		pos->minorPieceNum[index] = 0;
...
void PrintBoard(const CHESS_BOARD *pos) {
