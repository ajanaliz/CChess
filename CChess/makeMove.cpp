...
static void ClearPiece(const int sq, CHESS_BOARD *pos) {
	...
	if (PieceBig[pce]) {
		pos->bigPieceNum[col]--;
		if (PieceMaj[pce]) {
			pos->majorPieceNum[col]--;
		}
		else {
			pos->minorPieceNum[col]--;
		}
	}
	else {
...
static void AddPiece(const int sq, CHESS_BOARD *pos, const int pce) {
...
static void MovePiece(const int from, const int to, CHESS_BOARD *pos) {
...
int MakeMove(CHESS_BOARD *pos, int move) {
